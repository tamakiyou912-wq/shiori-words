...
- 罗马字输入可一键切换平假名、片假名或按读音联想汉字；单词发音使用设备内置日语语音，不消耗 AI。
...
Provider 请求使用低温度、关闭长推理并要求一个完整 JSON 对象。服务端先严格解析，再对轻微格式错误做本地修复；单个可选字段失败不会丢弃整条结果，且不会为了修 JSON 再调用一次 AI。浏览器仍通过逐行应用事件先收到本地结果，但不会解析未完成的模型 JSON。
